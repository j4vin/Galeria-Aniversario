<script lang="ts">
  ...
  // Ocultamos solo las imágenes de galería que serán animadas
  gsap.set(animatedGalleryItems, { autoAlpha: 0 });
  ...
  entryTl.then(() => {
    ...
    // Capturamos el estado inicial de las imágenes orbitales
    const state = Flip.getState(heroImages, {props: "rotation"});
    
    // Mostramos las imágenes de galería (todavía transparentes)
    gsap.set(animatedGalleryItems, { autoAlpha: 1 });
    ...
    scrollTl.add(
      Flip.from(state, {
        ...
        stagger: 0.05,
        absolute: true,
        onEnter: elements => gsap.fromTo(elements, {autoAlpha: 0}, {autoAlpha: 1, duration: 0.5}),
        onLeave: elements => gsap.to(elements, {autoAlpha: 0, duration: 0.5})
      }),
      '<'
    );
...
<div class="hero-container" bind:this={heroContainer}>
	...
	<div class="orbit-images">
		{#each animatedImages as image, i (image.id)}
            <div class="hero-image" bind:this={heroImages[i]} data-flip-id={`image-${image.id}`}>
