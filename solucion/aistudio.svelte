...
<main>
	...
	<div class="image-gallery">
		{#each images as image, i (image.id)}
			<!-- AÑADIDO: Asignamos el MISMO ID de Flip a las imágenes correspondientes en la galería -->
			<!-- Usamos un #if para aplicarlo solo a las 6 primeras que se animan -->
			<button
				class="gallery-item"
				on:click={() => openModal(image)}
				bind:this={galleryItems[i]}
				data-flip-id={i < animatedImages.length ? `img_${image.id}` : null}
			>
				<img src={image.src} alt={image.alt} />
			</button>
		{/each}
